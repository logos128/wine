...
NTSTATUS WINAPI pdo_ioctl(DEVICE_OBJECT *device, IRP *irp)
{
    ...
    switch ((code = irpsp->Parameters.DeviceIoControl.IoControlCode))
    {
        ...
        case IOCTL_HID_GET_INDEXED_STRING:
        {
            ...
            {
                if (!(langid = InterlockedOr(&ext->u.pdo.langid, 0)))
                {
                    hid_get_indexed_string(ext->u.pdo.parent_fdo, 0, buf, sizeof(buf), &io);
                    if (!io.Status && io.Information >= sizeof(buf[0]) * 2)  /* NULL terminated buffer */
                    {
                        ...
                        InterlockedExchange(&ext->u.pdo.langid, langid);
                    }
                }
            ...
            hid_get_indexed_string(ext->u.pdo.parent_fdo, index, output_buf, output_len, &irp->IoStatus);
