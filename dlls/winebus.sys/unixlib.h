...
{
    if (!desc) return "(null)";
    return wine_dbg_sprintf("{vid %04x, pid %04x, version %04x, input %d, uid %08x, bus_type %u, bus_id %08x, bus_num %u, port_path %u.%u.%u.%u.%u.%u.%u.%u, is_gamepad %u, is_hidraw %u}",
                            desc->vid, desc->pid, desc->version, desc->input, desc->uid,
                            desc->bus_type, desc->bus_id, desc->bus_num,
                            *(UINT8 *)&desc->port_path, *((UINT8 *)&desc->port_path + 1),
