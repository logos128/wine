...
{
    if (!desc) return "(null)";
    return wine_dbg_sprintf("{vid %04x, pid %04x, version %04x, input %d, uid %08x, busnum %u, portnum %u, usage %04x:%04x, is_gamepad %u, is_hidraw %u, is_bluetooth %u}",
                            desc->vid, desc->pid, desc->version, desc->input, desc->uid, desc->busnum, desc->portnum,
                            desc->usages.UsagePage, desc->usages.Usage, desc->is_gamepad, desc->is_hidraw, desc->is_bluetooth);
}
