...
static struct func_device *find_func_device(struct device_desc *desc)
{
    ...
    LIST_FOR_EACH_ENTRY(fdo, &fdo_list, struct func_device, entry)
    {
        if (fdo->base.desc.vid == desc->vid && fdo->base.desc.pid == desc->pid &&
            ...
            fdo->base.desc.bus_num == desc->bus_num && fdo->base.desc.port_path == desc->port_path)
            return fdo;
    }
...
static WCHAR *get_compatible_ids(DEVICE_OBJECT *device)
{
    ...
    if (ext->base.desc.bus_type == BUS_TYPE_USB && ext->base.desc.bus_id != -1)
    {
        UINT class = (ext->base.desc.bus_id >> 16) & 0xff, subclass = (ext->base.desc.bus_id >> 8) & 0xff,
             protocol = ext->base.desc.bus_id & 0xff;
...
static NTSTATUS WINAPI driver_add_device(DRIVER_OBJECT *driver, DEVICE_OBJECT *pdo)
{
    ...
    static const WCHAR instance_format[] = L"%*u&%u&%u&%llx";
    BOOL is_root = FALSE;
    ...
    result = swscanf(instance_id, instance_format, &fdo->base.desc.version,
                     &fdo->base.desc.bus_num, &fdo->base.desc.port_path);
    if (result < 3)
    {
        fdo->base.desc.version = fdo->base.desc.bus_num = 0;
        fdo->base.desc.port_path = 0;
    }
