...
static BOOL CALLBACK check_ffb_axes( const DIDEVICEOBJECTINSTANCEW *obj, void *args )
{
    ...
    {
        if (IsEqualIID(&obj->guidType, &GUID_XAxis))
            *value |= ForceFeedbackEffectAxes_X;
        else if (IsEqualIID(&obj->guidType, &GUID_YAxis))
            *value |= ForceFeedbackEffectAxes_Y;
        else if (IsEqualIID(&obj->guidType, &GUID_ZAxis))
            *value |= ForceFeedbackEffectAxes_Z;
    }
    return (*value == (ForceFeedbackEffectAxes_X | ForceFeedbackEffectAxes_Y | ForceFeedbackEffectAxes_Z)) ?
